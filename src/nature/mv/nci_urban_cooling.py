...
from natcap.invest import gettext, spec_utils, utils, validation
...
MODEL_SPEC = {
    "model_name": NCI_METADATA["urban_cooling_mv"].model_title,
    "pyname": NCI_METADATA["urban_cooling_mv"].pyname,
    # "userguide": NCI_METADATA["urban_cooling_mv"].userguide,
...
# Rebuild of InVEST Urban Cooling Model
_INDEX_NODATA = -999.0
...
def execute(args):
    ...
    """
    LOGGER.info("Starting NCI Urban Cooling Model")
    ...
    utils.make_directories([workspace_dir, intermediate_dir])
    ...
    LOGGER.debug(f"Scenarios: {scenario_labels_list}")
    ...
    cc_weight_sum = sum((cc_weight_shade_raw, cc_weight_albedo_raw, cc_weight_eti_raw))
    ...
    task_graph = taskgraph.TaskGraph(workspace_dir / "taskgraph_cache", n_workers)

    # align all the input rasters.
    for raster_type, raster_name in {
        "lulc_raster_path": "lulc",
        "ref_eto_raster_path": "ref_eto",
    }.items():
        scenario_args_dict[f"aligned_{raster_type}"] = {
            scenario_name: intermediate_dir / f"{raster_name}{file_suffix}.tif"
            for scenario_name in scenario_labels_list
        }

    lulc_raster_info = pygeoprocessing.get_raster_info(args["lulc_raster_path"])
    ...
    base_raster_path_list = [
        str(scenario_args_dict[f"lulc_raster_path"][scenario_name])
        for scenario_name in scenario_labels_list
    ] + [
        str(scenario_args_dict[f"ef_eto_raster_path"][scenario_name])
        for scenario_name in scenario_labels_list
    ]
    aligned_raster_path_list = [
        str(scenario_args_dict[f"aligned_lulc_raster_path"][scenario_name])
        for scenario_name in scenario_labels_list
    ] + [
        str(scenario_args_dict[f"aligned_ref_eto_raster_path"][scenario_name])
        for scenario_name in scenario_labels_list
    ]
    align_task = task_graph.add_task(
        ...
        args=(
            base_raster_path_list,
            aligned_raster_path_list,
            ["mode"] * len(scenario_labels_list)
            + ["cubicspline"] * len(scenario_labels_list),
            (cell_size, -cell_size),
            "intersection",
        ),
        kwargs={
    ...
    # TODO Iterate through scenarios
    task_path_prop_map = {name: {} for name in scenario_labels_list}
    scenario_variables = {
        scenario_variable_list: {name: {} for name in scenario_labels_list}
        for scenario_variable_list in [
            # TODO fill in this list with variables required for MV calculation
        ]
    }
    ...
    for prop in reclassification_props:
        prop_raster_path = intermediate_dir / f"{prop}{file_suffix}.tif"
        prop_task = task_graph.add_task(
            func=utils.reclassify_raster,
            args=(
                (scenario_args_dict[f"aligned_ref_eto_raster_path"][scenario_name], 1),
                biophysical_df[prop].to_dict(),
                prop_raster_path,
                gdal.GDT_Float32,
                TARGET_NODATA,
                reclass_error_details,
        ...
        task_path_prop_map[scenario_name][prop] = (prop_task, prop_raster_path)

    green_area_decay_kernel_distance = int(
        numpy.round(float(args["green_area_cooling_distance"]) / cell_size)
    )
    cc_park_raster_path = intermediate_dir / f"cc_park{file_suffix}.tif"
    cc_park_task = task_graph.add_task(
        func=nature.convolve_2d_by_exponential,
        args=(
            task_path_prop_map[scenario_name]["green_area"][1],
            cc_park_raster_path,
            green_area_decay_kernel_distance,
        ),
        target_path_list=[cc_park_raster_path],
        dependent_task_list=[task_path_prop_map[scenario_name]["green_area"][0]],
        task_name="calculate T air",
    )

    # Calculate the area of greenspace within a search radius of each pixel.
    area_kernel_path = intermediate_dir / f"area_kernel{file_suffix}.tif"
    ...
    green_area_sum_raster_path = intermediate_dir / f"green_area_sum{file_suffix}.tif"
    green_area_sum_task = task_graph.add_task(
        func=pygeoprocessing.convolve_2d,
        args=(
            (task_path_prop_map[scenario_name]["green_area"][1], 1),  # green area path
            (area_kernel_path, 1),
            green_area_sum_raster_path,
        ),
        kwargs={"working_dir": intermediate_dir, "ignore_nodata_and_edges": True},
        target_path_list=[green_area_sum_raster_path],
        dependent_task_list=[
            task_path_prop_map[scenario_name]["green_area"][
                0
            ],  # reclassed green area task
            area_kernel_task,
        ],
    ...
    cc_raster_path = intermediate_dir / f"cc{file_suffix}.tif"
    if args["cc_method"] == "factors":
        ...
        eto_nodata = pygeoprocessing.get_raster_info(args["ref_eto_raster_path"])[
            "nodata"
        ][0]
        eti_raster_path = intermediate_dir / f"eti{file_suffix}.tif"
        eti_task = task_graph.add_task(
            func=pygeoprocessing.raster_calculator,
            args=(
                [
                    (task_path_prop_map[scenario_name]["kc"][1], 1),
                    (TARGET_NODATA, "raw"),
                    (aligned_ref_eto_raster_path, 1),
                    (eto_nodata, "raw"),
                    (ref_eto_max, "raw"),
                    (TARGET_NODATA, "raw"),
                ],
            ...
            dependent_task_list=[task_path_prop_map[scenario_name]["kc"][0]],
        ...
        cc_task = task_graph.add_task(
            ...
            args=(
                [
                    (task_path_prop_map[scenario_name]["shade"][1], 1),
                    (task_path_prop_map[scenario_name]["albedo"][1], 1),
                    (eti_raster_path, 1),
                    (cc_weight_shade, "raw"),
            ...
            dependent_task_list=[
                task_path_prop_map[scenario_name]["shade"][0],
                task_path_prop_map[scenario_name]["albedo"][0],
                eti_task,
            ],
        ...
        # function.
        LOGGER.info("Calculating Cooling Coefficient using " "building intensity")
        cc_task = task_graph.add_task(
            func=pygeoprocessing.raster_calculator,
            args=(
                [(task_path_prop_map[scenario_name]["building_intensity"][1], 1)],
                calc_cc_op_intensity,
                cc_raster_path,
                gdal.GDT_Float32,
                TARGET_NODATA,
            ),
            target_path_list=[cc_raster_path],
            dependent_task_list=[
                task_path_prop_map[scenario_name]["building_intensity"][0]
            ],
    ...
    hm_raster_path = workspace_dir / f"hm{file_suffix}.tif"
    ...
    t_air_nomix_raster_path = intermediate_dir / f"T_air_nomix{file_suffix}.tif"
    ...
    decay_kernel_distance = int(numpy.round(t_air_average_radius_raw / cell_size))
    t_air_raster_path = intermediate_dir / f"T_air{file_suffix}.tif"
    t_air_task = task_graph.add_task(
        func=nature.convolve_2d_by_exponential,
        args=(t_air_nomix_raster_path, t_air_raster_path, decay_kernel_distance),
        target_path_list=[t_air_raster_path],
        dependent_task_list=[t_air_nomix_task],
        task_name="calculate T air",
    )

    cc_aoi_stats_pickle_path = intermediate_dir / "cc_ref_aoi_stats.pickle"
    _ = task_graph.add_task(
        func=pickle_zonal_stats,
        args=(intermediate_aoi_vector_path, cc_raster_path, cc_aoi_stats_pickle_path),
    ...
    t_air_aoi_stats_pickle_path = intermediate_dir / "t_air_aoi_stats.pickle"
    _ = task_graph.add_task(
        func=pickle_zonal_stats,
        args=(
            intermediate_aoi_vector_path,
            t_air_raster_path,
            t_air_aoi_stats_pickle_path,
        ),
    ...
    if bool(args["do_productivity_valuation"]):
        ...
        # work productivity
        wbgt_raster_path = intermediate_dir / f"wbgt{file_suffix}.tif"
        wbgt_task = task_graph.add_task(
            func=calculate_wbgt,
            args=(float(args["avg_rel_humidity"]), t_air_raster_path, wbgt_raster_path),
            target_path_list=[wbgt_raster_path],
            dependent_task_list=[t_air_task],
            task_name="vapor pressure",
        )

        light_work_loss_raster_path = (
            intermediate_dir / f"light_work_loss_percent{file_suffix}.tif"
        )
        heavy_work_loss_raster_path = (
            intermediate_dir / f"heavy_work_loss_percent{file_suffix}.tif"
        )
        ...
        # pickle WBGT
        wbgt_stats_pickle_path = intermediate_dir / "wbgt_stats.pickle"
        _ = task_graph.add_task(
            func=pickle_zonal_stats,
            args=(
                intermediate_aoi_vector_path,
        ...
        # pickle light loss
        light_loss_stats_pickle_path = intermediate_dir / "light_loss_stats.pickle"
        _ = task_graph.add_task(
            func=pickle_zonal_stats,
            args=(
                intermediate_aoi_vector_path,
                loss_task_path_map["light"][1],
        ...
        heavy_loss_stats_pickle_path = intermediate_dir / "heavy_loss_stats.pickle"
        _ = task_graph.add_task(
            func=pickle_zonal_stats,
            args=(
                intermediate_aoi_vector_path,
                loss_task_path_map["heavy"][1],
    ...
    if bool(args["do_energy_valuation"]):
        ...
        intermediate_building_vector_path = (
            intermediate_dir / f"reprojected_buildings{file_suffix}.shp"
        )
        ...
        t_air_stats_pickle_path = intermediate_dir / "t_air_stats.pickle"
        ...
        energy_consumption_vector_path = (
            workspace_dir / f"buildings_with_stats{file_suffix}.shp"
        )
        _ = task_graph.add_task(
            ...
            dependent_task_list=[pickle_t_air_task, intermediate_building_vector_task],
    ...
    target_uhi_vector_path = workspace_dir / f"uhi_results{file_suffix}.shp"
    _ = task_graph.add_task(
...
def calculate_uhi_result_vector(
    ...
    """
    LOGGER.info(
        "Calculate UHI summary results for "
    ...
    if light_loss_stats_pickle_path:
        ...
        with open(light_loss_stats_pickle_path, "rb") as (light_loss_stats_pickle_file):
    ...
    if heavy_loss_stats_pickle_path:
        ...
        with open(heavy_loss_stats_pickle_path, "rb") as (heavy_loss_stats_pickle_file):
    ...
    shapefile_driver.CreateCopy(target_uhi_vector_path, base_aoi_vector)
    ...
    if energy_consumption_vector_path:
        ...
        building_shapely_polygon_lookup = dict(
            (
                poly_feat.GetFID(),
                shapely.wkb.loads(bytes(poly_feat.GetGeometryRef().ExportToWkb())),
            )
    ...
    for feature in target_uhi_layer:
        ...
        if cc_stats[feature_id]["count"] > 0:
            mean_cc = cc_stats[feature_id]["sum"] / cc_stats[feature_id]["count"]
            feature.SetField("avg_cc", mean_cc)
        mean_t_air = None
        if t_air_stats[feature_id]["count"] > 0:
            mean_t_air = (
                t_air_stats[feature_id]["sum"] / t_air_stats[feature_id]["count"]
            )
            feature.SetField("avg_tmp_v", mean_t_air)

        if mean_t_air:
            feature.SetField("avg_tmp_an", mean_t_air - t_ref_val)

        if wbgt_stats and wbgt_stats[feature_id]["count"] > 0:
            wbgt = wbgt_stats[feature_id]["sum"] / wbgt_stats[feature_id]["count"]
        ...
        if energy_consumption_vector_path:
            ...
            aoi_shapely_geometry = shapely.wkb.loads(bytes(aoi_geometry.ExportToWkb()))
            aoi_shapely_geometry_prep = shapely.prepared.prep(aoi_shapely_geometry)
            avd_eng_cn = 0
            for building_fid in poly_rtree_index.intersection(
                ...
                if aoi_shapely_geometry_prep.intersects(
                    ...
                    energy_consumption_value = energy_consumption_layer.GetFeature(
                        building_fid
                    ).GetField("energy_sav")
...
def calculate_energy_savings(
    ...
    base_building_vector = gdal.OpenEx(base_building_vector_path, gdal.OF_VECTOR)
    shapefile_driver = gdal.GetDriverByName("ESRI Shapefile")
    ...
    try:
        ...
        pass
    shapefile_driver.CreateCopy(target_building_vector_path, base_building_vector)
    ...
    target_building_layer.CreateField(ogr.FieldDefn("energy_sav", ogr.OFTReal))
    target_building_layer.CreateField(ogr.FieldDefn("mean_t_air", ogr.OFTReal))
    ...
    fieldnames = [field.GetName().lower() for field in target_building_layer.schema]
    ...
    for target_index, target_feature in enumerate(target_building_layer):
        last_time = _invoke_timed_callback(
            ...
            lambda: LOGGER.info(
                ...
                100 * float(target_index + 1) / target_building_layer.GetFeatureCount(),
            ),
            _LOGGING_PERIOD,
        )
        ...
        if feature_id in t_air_stats:
            ...
            if pixel_count > 0:
                t_air_mean = float(t_air_stats[feature_id]["sum"] / pixel_count)
...
def pickle_zonal_stats(base_vector_path, base_raster_path, target_pickle_path):
    ...
    """
    LOGGER.info(
        f"Taking zonal statistics of {base_vector_path} " f"over {base_raster_path}"
    )
    zonal_stats = pygeoprocessing.zonal_statistics(
        (base_raster_path, 1), base_vector_path, polygons_might_overlap=True
    )
...
def calc_eti_op(kc_array, kc_nodata, et0_array, et0_nodata, et_max, target_nodata):
    ...
    result[valid_mask] = kc_array[valid_mask] * et0_array[valid_mask] / et_max
    return result


def calculate_wbgt(avg_rel_humidity, t_air_raster_path, target_vapor_pressure_path):
    ...
    def wbgt_op(avg_rel_humidity, t_air_array):
        ...
        e_i = (
            (avg_rel_humidity / 100)
            * 6.105
            * numpy.exp(17.27 * (t_air_valid / (237.7 + t_air_valid)))
        )
...
def hm_op(cc_array, green_area_sum, cc_park_array, green_area_threshold):
    ...
    result[:] = TARGET_NODATA
    valid_mask = ~(
        utils.array_equals_nodata(cc_array, TARGET_NODATA)
        & utils.array_equals_nodata(cc_park_array, TARGET_NODATA)
    )
    cc_mask = (cc_array >= cc_park_array) | (green_area_sum < green_area_threshold)
...
def map_work_loss(
    work_temp_threshold_array, temperature_raster_path, work_loss_raster_path
):
    ...
    """
    LOGGER.info(f"Calculating work loss using thresholds: {work_temp_threshold_array}")
    ...
    def classify_to_percent_op(temperature_array):
        ...
        valid_mask = ~utils.array_equals_nodata(temperature_array, TARGET_NODATA)
        result[valid_mask & (temperature_array < work_temp_threshold_array[0])] = 0
        result[
            valid_mask
        ...
        result[
            valid_mask
            & (temperature_array >= work_temp_threshold_array[1])
            & (temperature_array < work_temp_threshold_array[2])
        ] = 50
        result[valid_mask & (temperature_array >= work_temp_threshold_array[2])] = 75
        return result

    pygeoprocessing.raster_calculator(
        [(temperature_raster_path, 1)],
        classify_to_percent_op,
...
def validate(args, limit_to=None):
    ...
    if "biophysical_table_path" not in invalid_keys and "cc_method" not in invalid_keys:
        if args["cc_method"] == "factors":
